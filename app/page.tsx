...
export default function Home() {
  ...
  const [quickLinks, setQuickLinks] = useState<Array<{ url: string; title: string; favicon: string }>>([])
  ...
  useEffect(() => {
    ...
    const savedQuickLinks = localStorage.getItem('neura-quick-links')
    if (savedQuickLinks) {
  ...
  const handleQuickLinksChange = (newLinks: Array<{ url: string; title: string; favicon: string }>) => {
    setQuickLinks(newLinks)
    localStorage.setItem('neura-quick-links', JSON.stringify(newLinks))
  }
  ...
  return (
    ...
    >
      ...
      <main className="flex flex-col items-center justify-center min-h-screen px-4 py-12">
        ...
        {/* Quick Links */}
        {personalization.showQuickLinks && (
          <QuickLinks 
            links={quickLinks} 
            onLinksChange={handleQuickLinksChange}
          />
        )}
      </main>
    </div>
